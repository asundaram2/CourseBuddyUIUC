...
function FindBuddies(props) {
    ...
    function handleInfoSubmit() {
      ...
      console.log(email, grade, props.name, major)
      for (var key in courses) {
        if (courses[key] && courses[key].length > 0) {
          console.log("CC ", courses[key])
          axios
            ...
            .then(
              (response) => {
                console.log(response.status," Added ", response.data.crn)
                setInfoFilled(true);
              },
              (error) => {
                console.log("Failed to add ", courses[key])
                console.log(error);
              }
            );
        }
