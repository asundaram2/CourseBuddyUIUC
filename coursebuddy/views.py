...
def LoginAPI(request):
    ...
    if request.method == 'GET':
        ...
        try:
            name = Login.objects.raw('SELECT * FROM coursebuddy.login WHERE username= %s AND password  = %s;',[username,password])[0]
...
def ProfessorUserRatingsAPI(request):
    ...
    if request.method == 'GET':
        ...
        try:
            queryset = Test.objects.raw('''SELECT CRN,a.Professor,AverageGpa,avg_ratings,avg_difficulty
                                            FROM coursebuddy.sp19 as a JOIN (SELECT Professor,avg(Professor_ratings) as avg_ratings,avg(Difficulty) as avg_difficulty
                                                                                FROM coursebuddy.Professor_ratings_from_user as a join coursebuddy.sp19 as b on a.CRN = b.CRN
                                            WHERE a.CRN = %s group by Professor) as b on a.Professor = b.Professor''',[crn])
            q = queryset[0]
...
def AddUserRatingsAPI(request):
    ...
    if serializer.is_valid():
        s = serializer.data
        with connection.cursor() as cursor:
            cursor.execute('''INSERT INTO coursebuddy.Professor_ratings_from_user (Username, CRN, Professor_ratings, Difficulty)
                            VALUES (%s, %s, %s, %s);''',[s['username'],s['crn'],s['professor_ratings'],s['difficulty']])
...
def DeleteRatingsAPI(request):
    ...
    if request.method == 'GET':
        ...
        try:
            res = ProfessorRatingsFromUser.objects.raw('''SELECT *
                                                          FROM coursebuddy.Professor_ratings_from_user
                                                          WHERE Username=%s and CRN = %s;''',[user,crn])
            with connection.cursor() as cursor:
    ...
    for elems in profs:
        ...
        if name == searchName:
            ...
            try:
                ...
                ratingCount += 1
                print("RatingFound:", rating, ratingCount)
            except ValueError:
                print("ValueError")
                continue
    try:
...
def ProfessorWebsiteRatingsAPI(request):
    ...
    if request.method == 'GET':
        with connection.cursor() as cursor:
            print("Searching RateMyProfessors")
            print("Professor Name:", professor)
            ...
            if float(webdata['professor_ratings']) > 0:
                ...
                print("Checking DB for a rating")
                ...
                if results == []:
                    ...
                    print("RSD:", ratingSerialize.data)
